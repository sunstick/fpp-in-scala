...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      val more: Stream[(Block, List[Move])] = for {
        (block, moves) <- initial
        neighbors = neighborsWithHistory(block, moves)
   ...
   */
  lazy val pathsFromStart: Stream[(Block, List[Move])] = Stream((startBlock, Nil))

  /**
   * Returns a stream of all possible pairs of the goal block along
   * with the history how it was reached.
   */
  lazy val pathsToGoal: Stream[(Block, List[Move])] = from(pathsFromStart, Set(startBlock)).filter {
